...
const Profile = (props) => {
    ...
    const submitPost =()=>{
      const text = element.current.value;
      alert(text);
    };
    return (
                            ...
                            <div className="post-new">
                                <div className="post-new-media">
                                    ...
                                    <div className="post-new-media-input">
                                        <input ref={element} type="text" className="uk-input" placeholder="What's Your Mind ? Dennis!"/>
                                    </div>
                                </div>
                                <hr/>
                                <div className="post-new-type">
                                    ...
                                    <button onClick={submitPost} className="button outline-light circle" type="button" style= {{ borderColor: `#e6e6e6`,  borderWidth: `1px`}}
                                       aria-expanded="false">Public</button>

                                </div>


                            </div>
                            {posts.map((post)=><Post key={post.id} message={post.message} like={post.likesCount}/>)}
                        </div>
